...
var getFixtures = async (league, country) => {
  try{
    ...
    var secondResponse = await axios.get(response.data.data.leagues[index].fixures + `&secret=${apiSecret}`)
    var fixtureData = secondResponse.data.data.fixtures.map((game)=> {
